...
export class Card {
    ...
    _getTamplate() {
        const card = document.querySelector("#newCardTemplate").cloneNode(true).querySelector(".card");
        return card;
    }
    ...
    renderCards(cardsData) {
        cardsData.forEach((obj) => {
            const cardTitle = obj.name;
            const cardImage = obj.link;
            const initialLikes = obj.likes;
            const cardOwner = obj.owner._id;
            const cardId = obj._id;
            this._addCard(cardTitle, cardImage, initialLikes, cardOwner, cardId);
        });
    }

    _addCard(cardTitle, cardImage, initialLikes, cardOwner, cardId) {
        const card = this._createCard(cardTitle, cardImage, initialLikes, cardOwner, cardId);
